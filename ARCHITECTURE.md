...
* **TOCTOU Permission Validation:** To prevent Time-Of-Check to Time-Of-Use privilege escalation via the fallback handle, the driver **does not** bypass credentials during reads. Every time a process invokes a `read()` on the pending inode, the driver calls the standard `inode_permission(inode, MAY_READ)` helper to validate the *current calling process's credentials* against the file permissions. If the caller lacks read access, the syscall returns `EACCES` or `EPERM`. Only after the check succeeds does the driver delegate the read to the internal `struct file *` reference.
