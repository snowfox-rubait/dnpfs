...
* **Symlinks (100% Metadata SSD Storage & PATH_MAX Support):**
  - **Core Invariant:** **All symlinks (both temporary transaction fallback links and standard POSIX symlinks) reside strictly on the Metadata SSD Device.** The primary Data HDD device stores zero symlink bytes, zero head seeks for symlinks, and zero data block allocations for symlinks.
  - **Inline Symlinks ($\le 128$ bytes):** Symlink targets up to 128 bytes are stored inline within the SSD inode payload (`symlink_target: [u8; 128]`), providing zero-allocation $O(1)$ lookups directly from metadata SSD RAM cache.
  - **Extended Symlinks ($129$ to $4096$ bytes `PATH_MAX` & Packed SSD Storage):** If a symlink target path exceeds 128 bytes (up to Linux `PATH_MAX` of 4096 bytes), `symlink_metadata_block` allocates space in **Packed Symlink Metadata Blocks** on the Metadata SSD. Each 4 KB metadata block packs multiple extended symlink targets (up to 32 entries per block), eliminating storage waste and zeroing physical fragmentation overhead.
...
**Deterministic Positional Group Hash Formula:**
