...
* **Symlinks (100% Metadata SSD Storage & PATH_MAX Support):**
  ...
  - **Extended Symlinks ($129$ to $4096$ bytes `PATH_MAX`):** If a symlink target path exceeds 128 bytes (up to Linux `PATH_MAX` of 4096 bytes), the inode allocates a dedicated 4 KB **Metadata Block on the Metadata SSD Device** via `symlink_metadata_block`. The path content is protected by the metadata SSD's xxHash3 superblock/journal checksums and redundant backup system, keeping all symlink operations 100% on the fast metadata SSD.
